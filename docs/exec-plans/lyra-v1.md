...
Orb smoothness update on 2026-08-18: reviewed browser-native canvas and WebGL orb references before replacing the jitter-prone CSS surface. The orb now draws its liquid boundary, radial colour blend, and glow in a native canvas `requestAnimationFrame` loop; microphone amplitude is interpolated each frame, so audio level changes do not fight CSS transforms or React render cadence. The surrounding interaction, state copy, match line/card, and reduced-motion behavior remain intact. No package was added. `web: npm run lint && npm run build` passed.

Canvas-orb refinement on 2026-08-18: expanded the listening surface to a more prominent, nearly full circular 3D sphere with Lyra violet/aqua lighting, subtler low-frequency boundary motion, and a responsive radial backdrop. The status line now keeps the elapsed listening counter and stop action aligned. This is presentation-only: browser-memory PCM capture, bounded progressive checks, API contracts, and catalog behavior are unchanged. `web: npm run lint && npm run build` passed.

Landing-orb spacing refinement on 2026-08-18: introduced intentional negative space between the center sphere and its two outer field rings, with matching Lyra aqua/violet borders and restrained active-state glow. The hero now includes a compact private-catalog/15-second detail and a slow ambient backdrop, while mobile sizing and reduced-motion behavior remain covered. No identification behavior changed. `web: npm run lint && npm run build` passed.
