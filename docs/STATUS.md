...
- The local catalog was rebuilt from the two source MP3s after stale fingerprint rows caused its saved positive clips to fail. The query-work safeguard now samples evenly across queries above 5,000 fingerprints instead of rejecting long recordings, preserving temporal frame positions. The evaluator counts `insufficient audio signal` as a no-match, allowing silence to be evaluated. A temporary ten-query local run returned 7/7 expected matches (clean, 5-second, quiet, noise, and phone-like variants) and 3/3 expected no-matches (unrelated music, silence, white noise), with no wrong matches or false positives; p50 162 ms and p95/p99 182 ms. `make verify` and `make benchmark` passed. The generated inputs and manifest were removed or remain Git-ignored; this is a local smoke corpus, not a legal reproducible holdout corpus.
- The public Identify screen now uses a centered, audio-reactive live-listening orb. It starts microphone capture only after a tap, scales and ripples from measured PCM input level, shows 15-second progress and final-check state, and retains the existing upload fallback and bounded 5/8/11/15-second identification flow. Catalog/admin UI and query-audio retention behavior are unchanged. `web: npm run lint && npm run build` passed.
- The public landing experience was fully refined around the listening action: a spacious editorial headline, subtle grid/gradient depth, a glass-like secondary upload card, simplified private-catalog navigation, and a compact three-step explanation. The design remains code-native CSS, responsive, and free of external font or image dependencies. `web: npm run lint && npm run build` passed.
- The orb now owns the complete identification state: it is fluid and subtly animated at rest, reacts to microphone level while listening, enters a different processing state for live checks and uploaded files, and shows match/no-match copy inside itself. No default side cards or waveform are shown. A single CSS line and the one real returned track card animate outward from the orb only after an accepted match; narrow screens omit that decorative card. Motion respects `prefers-reduced-motion`. `web: npm run lint && npm run build` passed.
- The orb's liquid surface now renders in a native canvas `requestAnimationFrame` loop. Each frame eases the latest microphone level toward the visual state, avoiding the competing React/CSS transform updates that made the prior CSS-only surface jitter. No rendering dependency was added; `web: npm run lint && npm run build` passed.
- The canvas orb was refined into a larger, near-full circular 3D surface using Lyra's violet/aqua lighting. Its animation uses lower-amplitude, slower boundary motion and an interpolated audio level; the landing composition adds restrained radial depth and responsive space without changing the microphone or matching flow. `web: npm run lint && npm run build` passed on 2026-08-18.
- The orb field now preserves deliberate negative space between its central sphere and two violet/aqua outer rings. The landing hero adds a compact privacy/time detail and a subtle ambient halo; it remains responsive and honors reduced-motion preferences. `web: npm run lint && npm run build` passed on 2026-08-18.
- Starting a new microphone run now suppresses the prior completed match while audio is being captured or checked, so a former track cannot be displayed as the current recording. `web: npm run lint && npm run build` passed on 2026-08-18.
- Removed the decorative rotating dashed orbit layers and their four dots from the listening orb; its central sphere, circular field rings, and post-match connector remain. `web: npm run lint && npm run build` passed on 2026-08-18.
