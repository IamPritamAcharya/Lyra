...
The current deterministic matcher queries the index once per deduplicated hash set and ranks candidates by aligned hits, distinct hashes, alignment span, frequency diversity, then temporal concentration. Its seed acceptance floors are six aligned hits, three distinct hashes, three query anchors, two distinct anchor-frequency bins, a four-frame alignment span, 2% usable-query-anchor coverage, and a three-hit lead over the runner-up track. If two tracks tie on aligned-hit score, the matcher returns no match rather than choosing an arbitrary winner. These are not accuracy claims and must be calibrated by Phase 7 evaluation.
